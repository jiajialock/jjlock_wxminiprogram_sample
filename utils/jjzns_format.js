 ...
 * call : 回调方法
 */
const getErrorInfo = (code, call = undefined) => {
  var params = {};
...
const handleEventRestart = (field, res, call, error = undefined) => {
  ...
  if (actionMaxNumber[field]){
    let fieldValue = actionMaxNumber[field] || 1;
    if (fieldValue > maxNumber) {
      if (res.errCode) {
        getErrorInfo(res.errCode, call);
      }
      if (errorHandle) {
        errorHandle(error);
        clearTimeout(globalData.currentTimer);
